//
//  NotificationSounds.m
//  foodify
//
//  Created by Govinda Biswas on 09/01/25.
//
...
// New Method: Get Default Notification Sound
RCT_REMAP_METHOD(getDefaultSound, getDefaultSoundWithResolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject)
{
    @try {
        // Example: Return a commonly used notification sound
        NSURL *defaultSoundURL = [NSURL URLWithString:@"/System/Library/Audio/UISounds/sms-received1.caf"];

        if (!defaultSoundURL) {
            reject(@"no_sound", @"Default notification sound not found", nil);
            return;
        }

        // Convert URL to sound details
        NSString *urlString = defaultSoundURL.absoluteString;
        NSMutableDictionary *defaultSound = [NSMutableDictionary dictionary];
        [defaultSound setObject:@"Default Notification Sound" forKey:@"title"];
        [defaultSound setObject:urlString forKey:@"url"];
        
        resolve(defaultSound);
    }
