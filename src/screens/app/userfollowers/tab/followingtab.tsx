import React, { Dispatch, Fragment, memo, SetStateAction, useEffect, useMemo, useState } from 'react';
...
const FollowingTab = ({ _user }: FollowersTab) => {
    ...
    const [following, setFollowing] = useState<IUserfollowers[]>([]);
    ...
    useEffect(() => {
        if (_user) {
            setIsLoading(true)
            getfollowing(_user, _query)
                .then((response) => {
                    if (response.status === 200) {
                        if (!Array.isArray(response.data.data)) return
                        setFollowing([...removeduplicate, ...response.data.data])
    ...
    const onFollow = async (_user: string) => {
        ...
        await FollowUser(_user as string)
            .then((response) => {
                if (response.status === 200) {
                    setFollowing((prev: IUserfollowers[] | []) => {
                        if (!prev) return prev
    ...
    const removeduplicate = removeDuplicateObject<IUserfollowers>(following)
    return (
        <Fragment>
            <FlatList
                onEndReached={() => {
...
export default memo(FollowingTab, (prevState, nextState) =>
