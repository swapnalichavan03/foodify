...
import React, { Component } from "react";
import { View, Platform, findNodeHandle, StyleProp, ViewStyle } from "react-native";
import RNViewShot from "./RNViewShot";
...
export function captureRef<T>(view: number | React.RefObject<T> | null, optionsObject?: Partial<Options>): Promise<string> {
  ensureModuleIsLoaded();
  if (
    view &&
    typeof view === "object" &&
...
export default class ViewShot extends Component<Props> {
  ...
  capture = (): Promise<string> =>
    this.firstLayoutPromise
      .then(() => {
        const { root } = this;
        if (!root) return new Promise(() => {}); // component is unmounted, you never want to hear back from the promise
        return captureRef(root, this.props.options);
      })
      .then(
        (uri: string) => {
  ...
  syncCaptureLoop = (captureMode: string | undefined) => {
    cancelAnimationFrame(this._raf);
    if (captureMode === "continuous") {
      let previousCaptureURI = "-"; // needs to capture at least once at first, so we use "-" arbitrary string
      const loop = () => {
